...
let chores = [];
let dailyPlan = []; // ordered array of chore IDs
...
function checkMidnightReset() {
    ...
    if (lastReset !== today) {
        // Roll unfinished plan chores back — just clear the plan (chores remain in main list)
        dailyPlan = [];
...
function updateDailyPlan() {
    ...
    // Filter out any IDs that no longer exist (chore was deleted)
...
function updateUI() {
    ...
    // Sort: starred first within each section
    const sorted = [...chores].sort((a, b) => {
        if (a.type !== b.type) return 0; // only sort within same type
        return (b.starred ? 1 : 0) - (a.starred ? 1 : 0);
    });

    // Group by type preserving star-first order
